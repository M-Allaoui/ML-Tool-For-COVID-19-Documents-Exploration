...
distortions = []
K = range(2, 50)
embedding = umap.UMAP(n_neighbors=50, min_dist=0.01).fit_transform(d)

for k in K:
    k_means = KMeans(n_clusters=k)
    k_means.fit(embedding)
    print("done for k= ", k)
    distortions.append(sum(np.min(cdist(embedding, k_means.cluster_centers_, 'euclidean'), axis=1)) / d.shape[0])
